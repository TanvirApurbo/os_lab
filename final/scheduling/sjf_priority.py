def findAvgTimeSJF_Priority(n, at, bt, prio):
    wt = [0] * n
    tat = [0] * n
    ct = [0] * n
    completed = [0] * n
    ...
    while completedCount < n:
        ...
        # pick highest priority; tie-breaker = shortest burst
        for i in range(n):
            if not completed[i] and at[i] <= time:
                if (prio[i] > maxPriority) or (prio[i] == maxPriority and bt[i] < minBT):
                    maxPriority = prio[i]
                    minBT = bt[i]
                    idx = i
...
def main():
    ...
    findAvgTimeSJF_Priority(n, at, bt, prio)
